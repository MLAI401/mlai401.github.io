/**
 * City-graph search algorithm engine for the Search Strategies lecture tab.
 *
 * This is a DIRECT, city-relabeled copy of the already-verified Playground
 * engine (demos/search_demo/graph.js) -- same GRAPH/NODE_LAYOUT shape, same
 * algorithm functions, byte-for-byte the same control flow. Only the node
 * IDs changed (A-H -> 8 US city codes) and the edges/costs/layout were
 * relabeled 1:1 onto that same abstract graph, so results can never diverge
 * from the trusted, independently-verified algorithm behavior.
 *
 ...
 * an ARBITRARY (start, goal) pair: for the default goal LAX it uses the
 * exact AIMA-consistent integer heuristic table; for any other goal it
 * falls back to the same Euclidean-distance formula the original engine
 * uses for non-default goals -- so picking any city as start/goal here
 * behaves exactly like picking any node in the Playground.
 */
(function() {
/**
...
function getHeuristic(node, goal) {
  ...
  if (goal === 'LAX' && DEFAULT_HEURISTICS_TO_G[node] !== undefined) {
...
function runSMA(start, goal, maxNodes = 6) {
  ...
  // ---------- Public namespace ----------
  window.CityEngine = {
    GRAPH, NODE_LAYOUT, DEFAULT_HEURISTICS_TO_G, getHeuristic,
    runBFS, runDFS, runUCS, runIDS, runAStar, runGreedy, runBIBF, runSMA,
    getPredecessorsAlphabetical, getSuccessorsAlphabetical, reconstructPath, getPathCost
  };
})();
